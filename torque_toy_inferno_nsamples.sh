...
nchains=20
ncores=20
nsamples=(200 600 1000 1400 1800)
vary_a1=False
kr=7
kb=3
runLearn=TRUE
...
apptainer run --bind ${infernolib} apptainer/updated_env.sif ${script} ${ndata} ${nchains} ${ncores} ${nsamples[${PBS_ARRAYID}-1]} ${runLearn} ${valfile} ${metadata} ${trainfile}
apptainer run --bind ${infernolib} apptainer/updated_env.sif ${script} ${ndata} ${nchains} ${ncores} ${nsamples[${PBS_ARRAYID}-1]} FALSE ${testfile} ${metadata} ${trainfile}
