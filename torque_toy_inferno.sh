...
apptainer run --bind ${infernolib} apptainer/updated_env.sif ${script} ${ndata} ${nchains} ${ncores} ${nlist[${PBS_ARRAYID}-1]} ${runLearn} ${testfile} ${metadata} ${trainfile}
apptainer run --bind ${infernolib} apptainer/updated_env.sif ${script} ${ndata} ${nchains} ${ncores} ${nlist[${PBS_ARRAYID}-1]} FALSE ${gridfile} ${metadata} ${trainfile}
