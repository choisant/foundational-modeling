...
inferno_dir <- paste0(parent_dir, "/calibrate/", sub('.csv$', '', basename(traindatafile)), 
                        "/nsamples-", nsamples, "_nchains-",
                        nchains, "_ndata-", ntrain, "_calibrated")
...
h5createDataset(h5file, 'xvalues', dims = c(2, ntest))
h5createDataset(h5file, 'confidence', dims = c(1, ntest))
...
h5write(t(test_df[c("x1", "x2")]),
        file = h5file, name = 'xvalues', index = list(NULL, NULL))
h5write(t(test_df[c("Confidence")]),
        file = h5file, name = 'confidence', index = list(NULL, NULL))
if ("color" %in% names(test_df)) {
