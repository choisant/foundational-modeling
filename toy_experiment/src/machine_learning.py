...
def fwd_pass_bnn_classifier(net, X:Tensor, y:Tensor, device, optimizer, batchsize:int, train=False):
    """
    This function controls the machine learning steps, depending on if we are in training mode or not.
    """
    if train:
...
def train_bnn_classifier(net, traindata, testdata, batchsize:int, epochs:int, device, optimizer, early_stopping:int=-1):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for data in dataset:
            ...
            X, y = data
            #print(X[0], y[0])
            acc, ce_loss, kl_loss, cost = fwd_pass_bnn_classifier(net, X, y, device, optimizer, batchsize, train=True)
            #acc, loss = test(net, testdata, size=size)
            if i % 10 == 0: #Record every ten batches
...
def test_bnn_classifier(net, data, device, optimizer, batchsize:int = 32):
    """
    Calculates the accuracy and the loss of the model for a random batch.
    """
    net.eval()
    ...
    X, y = next(iter(dataset)) #get a random batch
    val_acc, val_ce_loss, val_kl_loss, val_cost = fwd_pass_bnn_classifier(net, X, y, device, optimizer, batchsize, train=False)
