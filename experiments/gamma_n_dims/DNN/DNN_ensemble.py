...
for i in range(len(n_data)):
    ...
    for j in tqdm(range(20)):
        val_df, test_df, grid_df = train_ensemble(n_ensemble, n_data[i], bs_list[i])
    ...
    if (not os.path.isdir(f"predictions/{trainfile}/ensemble") ):
        os.mkdir(f"predictions/{trainfile}/ensemble")
    test_ensembles[i].to_csv(f"predictions/{trainfile}/ensemble/{testfile}_SequentialNet_{n_ensemble}ensembles_best_ndata-{n_data[i]}.csv")
    grid_ensembles[i].to_csv(f"predictions/{trainfile}/ensemble/grid_{tag}_SequentialNet_{n_ensemble}ensembles_best_ndata-{n_data[i]}.csv")
